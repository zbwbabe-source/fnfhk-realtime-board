...
- 목표달성률: ${detailedData.section1.achievement_rate}%
...
출력 형식:
{
  "main_summary": "매장별 매출은 당월실적 [수치]를 기록하며 목표달성률 [%]로 [평가]. 당시즌 판매는 판매율 [%]로 [재고회전 평가], 누적판매 [수치] 달성. 과시즌 재고는 현재 [수치] 잔존, 소진율 [%]로 [관리 평가]. [종합평가].",
  "key_insights": [
...
중요 규칙:
1. 지표명은 반드시 입력 데이터에 표시된 명칭 그대로 사용 (당월실적, 목표달성률, YoY, 당시즌판매율, 누적판매, 누적입고, 과시즌재고, 소진율, 정체재고비중)
2. 모든 수치는 반드시 명시 (HKD, %, K/M 단위 포함)
3. 추측/가정/전망 금지
4. 보고서체 사용 (~임, ~하고 있음)
5. main_summary는 4-5문장, 300자 이내
6. key_insights는 5-7개 불릿, 각 불릿 80자 이내
7. 평가는 명확하게 (양호함/안정적임/우수함/관리 필요함/주의 필요함 등)
